...
def collect_class_images(label, dest_folder="", engine="google", verbose=True):
    try:

        if verbose:
            print("Collecting:", label)

        image_bulker = ImageBulker(engine)
...
if __name__ == "__main__":
    ...
    if args.jobs == 1:
        ...
        for label in class_labels:
            collect_class_images(label, dest_folder=args.output_path, engine=args.engine, verbose=args.verbose)
        ...
        nb_jobs = (int)(np.minimum(args.jobs, nb_cores))
        _ = Parallel(n_jobs=nb_jobs)(delayed(collect_class_images)(label, args.output_path, args.engine, args.verbose) for label in class_labels)
